...
module "eks" {
  ...
  access_entries = {
    "argocd_${var.cluster_name}" = {
      cluster_name  = "${var.cluster_name}"
      principal_arn = "arn:aws:iam::${data.aws_caller_identity.kubefirst_mgmt.account_id}:role/argocd-mgmt-k1-gitops-ing"
      policy_associations = {
        argocdAdminAccess = {
  ...
  data_json = jsonencode(
    {
      ...
      argocd_role_arn        = "arn:aws:iam::${data.aws_caller_identity.kubefirst_mgmt.account_id}:role/argocd-mgmt-k1-gitops-ing"
    }
  )
}
