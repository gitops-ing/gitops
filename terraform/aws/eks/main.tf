...
module "eks" {
  ...
  eks_managed_node_groups = {
    ...
    default_node_group = {
      desired_size = tonumber("6")      # tonumber() is used for a string token value
      min_size     = tonumber("1")                 # tonumber() is used for a string token value
      max_size     = tonumber("6") + 10 # tonumber() is used for a string token value
  ...
  access_entries = {
    ...
    "argocd_mgmt-k1-gitops-ing" = {
      cluster_name  = "mgmt-k1-gitops-ing"
      principal_arn = "arn:aws:iam::126827061464:role/argocd-mgmt-k1-gitops-ing"
      username      = "arn:aws:iam::126827061464:role/argocd-mgmt-k1-gitops-ing"
      policy_associations = {
        view_deployments = {
    ...
    "atlantis_mgmt-k1-gitops-ing" = {
      cluster_name  = "mgmt-k1-gitops-ing"
      principal_arn = "arn:aws:iam::126827061464:role/atlantis-mgmt-k1-gitops-ing"
      username      = "arn:aws:iam::126827061464:role/atlantis-mgmt-k1-gitops-ing"
      policy_associations = {
        view_deployments = {
          policy_arn = "arn:aws:eks::aws:cluster-access-policy/AmazonEKSViewPolicy"
          access_scope = {
...
module "vpc_cni_irsa" {
  ...
  role_name             = upper("VPC-CNI-IRSA-mgmt-k1-gitops-ing")
...
module "aws_ebs_csi_driver" {
  ...
  role_name = upper("EBS-CSI-DRIVER-mgmt-k1-gitops-ing")
...
data "aws_iam_policy_document" "crossplane_custom_trust_policy" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${split("arn:aws:iam::126827061464:oidc-provider/", module.eks.oidc_provider_arn)[1]}:aud"
      values   = ["sts.amazonaws.com"]
    }

    condition {
      test     = "StringLike"
      variable = "${split("arn:aws:iam::126827061464:oidc-provider/", module.eks.oidc_provider_arn)[1]}:sub"
      values   = ["system:serviceaccount:crossplane-system:crossplane-provider-terraform-mgmt-k1-gitops-ing"]
    }
  ...
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::126827061464:role/KubernetesAdmin"]
    }
  }
  statement {
    effect  = "Allow"
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::126827061464:role/argocd-${local.name}"]
    }
  }
}
